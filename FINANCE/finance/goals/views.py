...
# ========== CONFIRM GOAL CREATION WITH SUGGESTED DEADLINE ==========
class ConfirmGoalCreationAPI(APIView):
    renderer_classes = [JSONRenderer]

    def post(self, request):
        """
        Create a goal with the AI-suggested deadline after user approval.
        """
        if 'user_id' not in request.session:
            return Response({"success": False, "error": "Authentication required"}, status=401)
        ...
        # Create the goal with approved deadline
        if not name:
            name = f"Goal {Goal.objects.filter(user=user).count() + 1}"
        ...
        # Apply cascade adjustment - re-evaluate all goals
        ...
        return Response({
            ...
            "message": f"Goal created! {len(adjustments)} other goals adjusted for feasibility." if adjustments else "Goal created successfully!"
        }, status=201)
    ...
    def get(self, request):
        ...
        for goal in active_goals:
            try:
                ...
                # Get current balance (either manually saved or calculated from savings history)
                ...
                active_with_analysis.append({
                    ...
                    'analysis': {
                        ...
                        'projected_balance_at_deadline': current_balance + (feasibility['actual_monthly'] * (remaining_days / 30)),
...
# ========== EXTEND GOAL DEADLINE (deprecated - now handled by ConfirmShiftAPI) ==========
# Removed - use ConfirmShiftAPI instead


# ========== MARK GOAL AS COMPLETED (deprecated - now handled by MarkAchievedAPI) ==========
# Removed - use MarkAchievedAPI instead


# ========== GOAL SUGGESTIONS (optional AI feature - keep for future) ==========
# Removed - can be re-added later if needed
